...
from src.domain.entities import TipoUsuario, CategoriaItem, EstadoItem
...
class TestSistemaCompleto(unittest.TestCase):
    ...
    def test_flujo_completo_prestamo_con_devolucion_tardia(self):
        ...
        prestamo_devuelto = self.prestamo_service.devolver_item(
            prestamo.id,
            "Devolución tardía"
        )
    ...
    def test_busqueda_y_filtros(self):
        ...
        libro1 = self.item_service.agregar_item(
        ...
        revista = self.item_service.agregar_item(
            titulo="Scientific American",
            categoria="revista",
            autor="Varios"
        )
        
        dvd = self.item_service.agregar_item(
            titulo="Cosmos Documentary",
            categoria="dvd",
    ...
    def test_autenticacion_completa(self):
        ...
        # Cambiar contraseña
        cambio_exitoso = self.auth_service.cambiar_password(
            empleado.id,
            "admin123",
            "nueva_password"
        )
    ...
    def test_validaciones_y_restricciones(self):
        ...
        empleado = self.auth_service.crear_empleado(
            ...
            usuario_sistema="test", password="test"
        )
        ...
        usuario1 = self.usuario_service.registrar_usuario(
            nombre="User1", apellido="Test", email="duplicate@test.com",
            tipo=TipoUsuario.ALUMNO, numero_identificacion="123"
        )
        
        with self.assertRaises(ValueError):
            self.usuario_service.registrar_usuario(
                nombre="User2", apellido="Test", email="duplicate@test.com",
                tipo=TipoUsuario.DOCENTE, numero_identificacion="456"
            )
        
        # Test: Usuario del sistema duplicado en empleados
        with self.assertRaises(ValueError):
            self.auth_service.crear_empleado(
                nombre="Otro", apellido="Empleado", email="otro@test.com",
                usuario_sistema="test", password="pass"  # Usuario ya existe
            )
        ...
        with self.assertRaises(ValueError):
            self.prestamo_service.realizar_prestamo(
                usuario_id=9999,  # Usuario inexistente
                item_id=item.id,
                empleado_id=empleado_actual.id
            )
        ...
        with self.assertRaises(ValueError):
            self.reserva_service.realizar_reserva(
                usuario_id=usuario1.id,
                item_id=item.id,  # Item disponible
                empleado_id=empleado_actual.id
            )
