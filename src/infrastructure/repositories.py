...
from ..domain.entities import Usuario, ItemBiblioteca, Prestamo, Reserva, Multa, Empleado, TipoUsuario, EstadoItem, CategoriaItem
from ..application.interfaces import IUsuarioRepository, IItemBibliotecaRepository, IPrestamoRepository, IReservaRepository, IMultaRepository, IEmpleadoRepository
...
class PrestamoRepository(IPrestamoRepository):
    ...
    def _row_to_entity(self, row: Dict[str, Any]) -> Prestamo:
        return Prestamo(
            ...
            fecha_devolucion_esperada=datetime.fromisoformat(row['fecha_devolucion_esperada']) if row['fecha_devolucion_esperada'] else None,
            fecha_devolucion_real=datetime.fromisoformat(row['fecha_devolucion_real']) if row['fecha_devolucion_real'] else None,
