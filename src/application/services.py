...
from ..domain.entities import Usuario, ItemBiblioteca, Prestamo, Reserva, Multa, EstadoItem, TipoUsuario, Empleado
from .interfaces import (
    IUsuarioRepository, IItemBibliotecaRepository, IPrestamoRepository, 
    IReservaRepository, IMultaRepository, IEmpleadoRepository
)
...
class UsuarioService:
    ...
    def registrar_usuario(self, nombre: str, apellido: str, email: str, 
                         tipo: TipoUsuario, numero_identificacion: str, 
...
class ItemBibliotecaService:
    ...
    def agregar_item(self, titulo: str, categoria: str, autor: Optional[str] = None,
                    isbn: Optional[str] = None, descripcion: Optional[str] = None,
                    ubicacion: Optional[str] = None, valor_reposicion: Optional[float] = None) -> ItemBiblioteca:
...
class PrestamoService:
    ...
    def realizar_prestamo(self, usuario_id: int, item_id: int, empleado_id: int,
                         dias_prestamo: int = 15) -> Prestamo:
...
class ReservaService:
    ...
    def realizar_reserva(self, usuario_id: int, item_id: int, empleado_id: int,
                        dias_expiracion: int = 3) -> Reserva:
        usuario = self.usuario_repo.obtener_por_id(usuario_id)
        if not usuario:
            raise ValueError(f"No se encontró el usuario con ID: {usuario_id}")
