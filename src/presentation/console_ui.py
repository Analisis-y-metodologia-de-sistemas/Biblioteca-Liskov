...
from ..domain.entities import Usuario, ItemBiblioteca, TipoUsuario, CategoriaItem
...
from ..shared.menu_utils import (
    select_from_list, show_dropdown_menu, MenuItem, confirm_action,
...
class ConsoleUI:
    ...
    def mostrar_menu_principal(self):
        ...
        return show_dropdown_menu(
            title="SISTEMA DE GESTIÓN BIBLIOTECA LISKOV",
            items=menu_items,
            allow_cancel=False
        )
        ...
        try:
            ...
            if confirm_action(f"¿Confirmar devolución del préstamo #{prestamo_seleccionado.id}?", default=True):
                ...
                if prestamo_actualizado.fecha_devolucion_real > prestamo_actualizado.fecha_devolucion_esperada:
                    dias_atraso = (prestamo_actualizado.fecha_devolucion_real - prestamo_actualizado.fecha_devolucion_esperada).days
    ...
    def historial_prestamos_usuario(self):
        ...
        try:
            ...
            if not prestamos:
                show_warning(f"No hay préstamos registrados para {usuario_seleccionado.nombre} {usuario_seleccionado.apellido}")
                return
            ...
            for prestamo in prestamos:
                ...
                fecha_dev = prestamo.fecha_devolucion_real.strftime('%d/%m/%Y') if prestamo.fecha_devolucion_real else "Pendiente"
    ...
    def realizar_reserva(self):
        ...
        try:
            ...
            if confirm_action(f"¿Confirmar reserva de '{item_seleccionado.titulo}' para {usuario_seleccionado.nombre}?", default=True):
    ...
    def registrar_pago_multa(self):
        ...
        try:
            ...
            if confirm_action(f"¿Confirmar pago de multa #{multa_seleccionada.id} por ${multa_seleccionada.monto:.2f}?", default=True):
        ...
        try:
            ...
            while True:
                ...
                item_seleccionado = show_dropdown_menu(
                    title=f"{categoria_nombre} ({len(items)} items)",
    ...
    def mostrar_detalles_item(self, item):
        ...
        try:
            ...
            print(f"📅 Fecha adquisición: {item.fecha_adquisicion.strftime('%d/%m/%Y') if item.fecha_adquisicion else 'No registrada'}")
